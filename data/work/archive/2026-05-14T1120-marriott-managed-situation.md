...
output: '## Agent run 2026-05-14T13:05Z
  ...
  retried once) "Reset Password Request" email from noreply@canarytechnologies.com.
  Either reset link works.

  Per Taylor''s note, this was never a Marriott-managed property so no hotel-level
  changes were needed (no SF dissociation, no slug archive, no new dashboard). She
  keeps the same Canary hotel and dashboard.

  @Ashley if Cindy doesn''t receive the email in ~10 min, check spam, then re-trigger
  via Django admin "Send password reset" on her user record. The dashboard URL for
