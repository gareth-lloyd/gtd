---
area: null
completed_at: null
contexts:
- consume
created: 2026-08-06 12:41:38.106116
defer_until: null
due: null
energy: low
id: 2026-08-06T1241-read-ihg-environment-onboarding-contract-chain-cod
order: null
output: ''
project: null
source_id: https://canarytechnologies.slack.com/archives/C03V5P4B48P/p1785937180532009?thread_ts=1785850461.696089&cid=C03V5P4B48P
tags:
- morning-gtd
- slack
- from-awareness
time_minutes: 10
title: Read IHG environment onboarding contract — chain code, OPERA URL, hotel code,
  Business Event Streaming
updated: 2026-08-06 12:41:38.170911
waiting_on: null
waiting_since: null
working_on: false
---

Taylor and Fred nailed down what IHG must send us: per-environment initial connection + Business Event Streaming setup, then chain code + environment for each new site (Fred adds OPERA URL to confirm data-centre location, and hotel code).
https://canarytechnologies.slack.com/archives/C03V5P4B48P/p1785937180532009?thread_ts=1785850461.696089&cid=C03V5P4B48P