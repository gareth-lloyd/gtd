---
area: null
completed_at: null
contexts:
- consume
created: 2026-09-16 16:04:19.951249
defer_until: null
due: null
energy: medium
id: 2026-09-16T1604-read-the-workup-catch-up-brief-before-tonight-s-la
order: 2
output: ''
project: 2026-09-08-workup
source_id: null
tags: []
time_minutes: 15
title: Read the Workup catch-up brief before tonight's Laura 1-1 (21:30) and the Workup
  sync (18:30)cF
updated: 2026-09-21 14:55:53.677904
waiting_on: null
waiting_since: null
working_on: false
---

Notion (private): https://app.notion.com/p/3dd8146861518127a5baca7c17b32a0e

Condensed state, strategy, next priorities, weaknesses, and per-meeting agendas (Laura 1-1, Workup sync, Asher skip-level Thu, Blake directs Mon, Internal Tools block review Thu 24 Sep).

Long version with every link is in the output of GTD item 2026-09-01T1716-i-need-to-be-more-on-top-of-workup-i-m-behind-the (project Workup).