...
class TestSearchNotes:
    ...
    def test_includes_trashed_when_requested(self, bear_db_factory, snapshot_dir):
        ...
        ids = {
            m.note.unique_id
            for m in reader.search_notes("match", include_trashed=True)
        }
        assert ids == {"a", "b"}
    ...
    def test_excludes_archived_when_requested(self, bear_db_factory, snapshot_dir):
        ...
        ids = {
            m.note.unique_id
            for m in reader.search_notes("match", include_archived=False)
        }
        assert ids == {"a"}
...
class TestRefresh:
    def test_changes_are_invisible_until_refresh(
        self, bear_db_factory, snapshot_dir
    ):
        from .conftest import make_bear_db
