...
export function TemplatesView() {
  ...
  if (list.length === 0) {
    return (
      <div className="empty">
        No recurring templates yet. Add one under{' '}
        <code>data/{env}/templates/</code> and it will appear here.
      </div>
    );
  }
  ...
  return (
    <ul className="template-list">
      {sorted.map((t) => (
        <li key={t.id}>
          <TemplateRow template={t} project={t.project ? projectsById.get(t.project) ?? null : null} />
        </li>
      ))}
    </ul>
  );
      ...
      <div className="template-meta">
        ...
        {template.contexts.map((c) => (
          <span
            ...
            style={contextChipStyle(c)}
          >
            @{c}
          </span>
        ))}
        {template.energy && <span className="chip">⚡{template.energy}</span>}
        {template.time_minutes != null && (
          <span className="chip">{template.time_minutes}m</span>
        )}
        {template.area && <span className="chip">{template.area}</span>}
        <span className="chip last-spawned">
          {template.last_spawned
            ? `last spawned ${template.last_spawned}`
            : 'never spawned'}
        </span>
      </div>
    </div>
